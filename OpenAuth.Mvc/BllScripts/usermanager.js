...
function resetTypeDesc() {
    var ids = $('#maingrid').getDataIDs();//返回数据表的ID数组["66","39"..]  
    var len = ids.length;
    for (var i = 0; i < len; i++) {
        var getRow = $('#maingrid').getRowData(ids[i]);//获取当前的数据行  
        if (getRow.Type == '0')
            $('#maingrid').jqGrid('setCell', i + 1, 'TypeDesc', '个人');
        else
            $('#maingrid').jqGrid('setCell', i + 1, 'TypeDesc', '企业');
    }
}